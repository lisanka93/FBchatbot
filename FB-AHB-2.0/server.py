...
ACCESS_TOKEN = "EAAIQ3AZBxreYBAE3BR6NtwG5xX7mAjNUIMs7tpsZBAUNOosEtuLDkSZBgwv5hQZCgSx7lXND5aFMnDCt2EBguja5loJXPMMYvw9ZApprKYiWlFbm3vwJCRAFsCayhe90y3OF62eknrOcCw6QF5Id5khZABBVvHWYRTii6bWlNEuwZDZD"
...
# *******************************  START CONVERSATION WITH ARGHHEALTHBOT ************************************
    ...
    if value_counters[key_id] == 1:
        if message in values_list:
            ...
            bot_reply = "sorry that is not from the list, please select from the list"
            reply(sender, bot_reply)
            return "okk"
    ...
    if len(chatlogs[key_id]) is 3:
        bot_reply = "great, lets get started. How often do you exercise (never for less than 60 minutes per week and sometimes for 60-150 minutes per week)?  "
    ...
    if len(chatlogs[key_id]) is 5 or checkpointlists[key_id][0] == "fail":
        ...
        if message == "sometimes":
            bot_reply = "Okay. What is the main reason why you only sometimes exercise. Please give only one reason for now"
            checkpointlists[key_id] = [1]
            checkpointlists[key_id].append(3)

        elif message == "never":
            bot_reply = "Okay. What is the main reason why you never exercise. Please give only one reason for now"
            checkpointlists[key_id] = [2]
            checkpointlists[key_id].append(3)

        else:
            bot_reply = "sorry I didn\'t get you, please select sometimes or never"
